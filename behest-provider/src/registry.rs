...
impl ProviderRegistry {
    ...
    {
        let id = provider.id();
        self.chat.insert(id.to_string(), Arc::new(provider))
    }
    ...
    pub fn register_chat_arc(
        ...
        self.chat.insert(provider.id().to_string(), provider)
    }
    ...
    {
        let id = provider.id();
        self.embeddings.insert(id.to_string(), Arc::new(provider))
    }
    ...
    pub async fn stream(
        ...
        provider_id: &ProviderId,
        request: ChatRequest,
    ) -> ProviderResult<crate::ChatStream> {
        let provider = self
            .chat(provider_id)
            .ok_or_else(|| unsupported(provider_id, "chat"))?;
