...
fn convert_message(message: &Message) -> OpenAiMessage {
    match message {
        ...
        _ => OpenAiMessage {
            role: "user".to_owned(),
            content: Some(Value::String("[unsupported message]".to_owned())),
            tool_calls: None,
            tool_call_id: None,
        },
    }
...
mod tests {
    ...
    #[test]
    fn serialize_content_should_not_use_mime_type_as_image_detail() {
        let parts = vec![ContentPart::image_url(
            "https://example.com/image.png",
            Some("image/png".to_owned()),
        )];
        let result = serialize_content(&parts);
        let image_url = &result.as_array().unwrap()[0]["image_url"];
