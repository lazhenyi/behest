//! High-level context factory API.
//!
//! This module was moved here from `behest::context` so the context
//! composition logic lives in the [`behest_context`] crate next to the
//! lower-level layered trait system.
...
use behest_core::error::ContextError;
use behest_provider::{ChatRequest, Message, ModelName, ToolChoice, ToolSpec};
...
impl ContextOutput {
    ...
    /// Returns a mutable reference to the composed messages.
    #[must_use]
...
impl ContextFactory {
    ...
    pub async fn build_request(
        ...
        let request = if let Some(specs) = tool_specs {
...
mod tests {
    ...
    async fn context_factory_should_build_request_with_tools() {
        ...
        let tools = vec![ToolSpec::new("echo", "Echo tool", json!({}))];
        let request = factory
            .build_request(&input, ModelName::new("gpt-4"), Some(&tools))
            .await
            .unwrap();
