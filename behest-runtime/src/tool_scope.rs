...
use behest_provider::ToolSpec;
use behest_tool::{Tool, ToolRegistry};
...
impl ScopedToolRegistry {
    ...
    /// # Errors
    ///
    /// Returns [`ToolError::NotFound`](behest_core::error::ToolError::NotFound)
    /// when the tool is not in any scope or the base.
    pub async fn execute(
        &self,
        call: &behest_provider::ToolCall,
    ) -> behest_tool::ToolResult<behest_tool::ToolOutput> {
        let tool = self
            .get(&call.name)
            .ok_or_else(|| behest_core::error::ToolError::NotFound {
...
mod tests {
    ...
    fn make_tool(name: &'static str) -> FunctionTool {
        FunctionTool::new(
            name,
            format!("{name} desc"),
            json!({"type": "object"}),
    ...
    async fn execute_should_resolve_from_scoped_registry() {
        ...
        let call = behest_provider::ToolCall::new("c1", "tool", json!({"x": 1}));
    ...
    async fn execute_unknown_tool_returns_not_found() {
        ...
        let call = behest_provider::ToolCall::new("c1", "missing", json!({}));
