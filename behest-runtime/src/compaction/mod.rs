...
use behest_provider::{ChatProvider, ChatRequest, Message, ModelName};
use behest_store::{CompactionMeta, MessageRecord, MessageRole, SessionStore};
...
pub struct CompactionService {
    ...
    config: crate::policy::CompactionConfig,
}
...
impl CompactionService {
    ...
    #[must_use]
    pub fn config(&self) -> &crate::policy::CompactionConfig {
        &self.config
    }
    ...
    async fn compact_impl(
        ...
        let compaction_user = MessageRecord {
            ...
            content: vec![behest_provider::ContentPart::text("[compaction]")],
            tool_calls: Vec::new(),
            tool_call_id: None,
            tool_name: None,
            usage: None,
        ...
        let summary_message = MessageRecord {
            ...
            content: vec![behest_provider::ContentPart::text(&summary_text)],
            tool_calls: Vec::new(),
            tool_call_id: None,
            tool_name: None,
            usage: None,
    ...
    async fn run_compaction_llm(
        ...
        if summary.is_empty() {
            return Err(RuntimeError::Provider(
                behest_core::error::ProviderError::Decode {
...
fn extract_text_content(message: &Message) -> String {
    match message {
        ...
        | Message::Tool { content, .. } => {
            ...
            for part in content {
                if let behest_provider::ContentPart::Text { text: t, .. } = part {
                    text.push_str(t);
                }
            }
            text
...
mod tests {
    ...
    use behest_store::{MessageRecord, MessageRole};
    use uuid::Uuid;
    ...
    fn make_user(text: &str) -> MessageRecord {
        MessageRecord::new(
            Uuid::now_v7(),
            MessageRole::User,
            vec![behest_provider::ContentPart::text(text)],
        )
    }
