...
fn turns(messages: &[MessageRecord]) -> Vec<Turn> {
    ...
    for (i, msg) in messages.iter().enumerate() {
        if !msg.is_compaction && msg.role == behest_store::MessageRole::User && !current.is_empty()
        {
            result.push(Turn {
...
mod tests {
    ...
    fn make_msg(role: behest_store::MessageRole, text: &str) -> MessageRecord {
        MessageRecord::new(Uuid::now_v7(), role, vec![ContentPart::text(text)])
    }

    fn make_user(text: &str) -> MessageRecord {
        make_msg(behest_store::MessageRole::User, text)
    }

    fn make_assistant(text: &str) -> MessageRecord {
        make_msg(behest_store::MessageRole::Assistant, text)
    }

    fn make_tool(text: &str) -> MessageRecord {
        make_msg(behest_store::MessageRole::Tool, text)
    }
    ...
    fn multiple_turns_preserves_recent() {
        ...
        assert_eq!(result.tail[0].role, behest_store::MessageRole::User);
    }
