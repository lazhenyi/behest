...
pub fn prune_tool_outputs(messages: &mut [MessageRecord], skip_turns: usize) -> PruneResult {
    ...
    for (i, msg) in messages.iter().enumerate().rev() {
        if !msg.is_compaction && msg.role == behest_store::MessageRole::User {
            turn_count += 1;
    ...
    for (idx, msg) in messages.iter().enumerate().rev() {
        ...
        if msg.role != behest_store::MessageRole::Tool {
            continue;
        }

        // Never prune protected tools
        if let Some(ref tool_name) = msg.tool_name
            && PRUNE_PROTECTED_TOOLS.contains(&tool_name.as_str())
        {
            continue;
        }
    ...
    for target in &targets {
        ...
        msg.is_summary = true;
        msg.content = vec![behest_provider::ContentPart::text(PRUNED_PLACEHOLDER)];
    }
...
mod tests {
    ...
    fn make_tool_msg(name: &str, output: &str) -> MessageRecord {
        MessageRecord {
            ...
            role: behest_store::MessageRole::Tool,
