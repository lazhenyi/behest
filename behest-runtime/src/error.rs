...
pub enum RuntimeError {
    ...
    #[error(transparent)]
    Provider(#[from] behest_core::error::ProviderError),

    /// An error propagated from the context layer.
    #[error(transparent)]
    Context(#[from] behest_core::error::ContextError),

    /// An error propagated from the storage layer.
    #[error(transparent)]
    Storage(#[from] behest_core::error::StorageError),

    /// An error propagated from the tool layer.
    #[error(transparent)]
    Tool(#[from] behest_core::error::ToolError),
