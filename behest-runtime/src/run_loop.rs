...
use behest_provider::{ChatRequest, ChatStreamEvent, FinishReason, Message, TokenUsage, ToolCall};
...
impl AgentRuntime {
    ...
    pub(super) async fn run_loop(
        ...
        run_id: RunId,
        session_id: Uuid,
        provider: Arc<dyn behest_provider::ChatProvider>,
        request: RunRequest,
        tool_specs: Vec<behest_provider::ToolSpec>,
        has_tools: bool,
        iteration: usize,
    ...
    async fn run_loop_inner(
        ...
        loop {
                ...
                || resume_from == TurnState::CallingModel
            {
                ...
                if let Some(u) = &usg {
                    total_usage = total_usage.merge(*u);
                    self.emit(&AgentEvent::UsageRecorded(UsageRecorded {
                        run_id,
                        usage: *u,
                        timestamp: Utc::now(),
                    }));
                    self.emit_cache_metrics(run_id, u).await;
                }
        ...
        if caps.chat_stream {
            match self.call_streaming(provider, request.clone(), run_id).await {
                ...
                Err(e)
                    if matches!(
                        e,
                        RuntimeError::Provider(behest_core::error::ProviderError::Timeout { .. })
                    ) =>
                {
                    return Err(e);
                }
        ...
        Message::Assistant { content, .. } => {
            let text: String = content
                ...
                .filter_map(|p| match p {
                    behest_provider::ContentPart::Text { text, .. } => Some(text.as_str()),
                    _ => None,
                })
                .collect();
