...
use super::component::{AnyComponent, AnyComponentError, Component, ComponentContext};
use super::extension::ExtensionError;
use super::registry::{ComponentDescriptor, ComponentFactory, RegistryError};
...
impl AnyComponent for ChatProviderAny {
    ...
    fn health(&self) -> BoxFuture<'_, behest_core::health::HealthStatus> {
        let inner = self.inner.clone();
...
mod tests {
    ...
    impl ChatProvider for StubChat {
        ...
        async fn complete(&self, _r: ChatRequest) -> ProviderResult<ChatResponse> {
            Ok(ChatResponse {
                provider: self.id(),
                model: behest_provider::ModelName::new("stub-model"),
    ...
    impl EmbeddingProvider for StubEmbedding {
        ...
        async fn embed(&self, _r: EmbeddingRequest) -> ProviderResult<EmbeddingResponse> {
            Ok(EmbeddingResponse {
                provider: self.id(),
                model: behest_provider::ModelName::new("stub-emb-model"),
                embeddings: Vec::new(),
                usage: None,
                raw: None,
            })
