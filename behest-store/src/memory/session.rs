...
impl SessionStore for MemorySessionStore {
    ...
    async fn update_session(
        ...
        let session = sessions
            .get_mut(id)
            .ok_or_else(|| behest_core::error::StorageError::NotFound { id: id.to_string() })?;
    ...
    async fn append_message(&self, message: MessageRecord) -> StoreResult<MessageRecord> {
        ...
        // Update session timestamp first (acquire sessions lock, release)
        {
            ...
            let session = sessions.get_mut(&session_id).ok_or_else(|| {
                behest_core::error::StorageError::NotFound {
    ...
    async fn update_usage(&self, message_id: &Uuid, usage: TokenUsage) -> StoreResult<()> {
        ...
        let session_id = {
            ...
            index.get(message_id).copied().ok_or_else(|| {
                behest_core::error::StorageError::NotFound {
        ...
        Err(behest_core::error::StorageError::NotFound {
            id: message_id.to_string(),
        })
    }
}
