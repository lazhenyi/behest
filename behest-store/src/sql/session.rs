        ...
        rows.into_iter()
            .map(
                |(
                    ...
                    Ok(MessageRecord {
                        id: crate::util::parse_uuid(&id, "message.id")?,
                        session_id: crate::util::parse_uuid(&sid, "message.session_id")?,
                        role: role_from_str(&role),
                        content: de_content(&content)?,
                        tool_calls: de_tool_calls(&tool_calls)?,
                        tool_call_id: None,
        ...
        rows.into_iter()
            .map(|(id, title, model, metadata, created_at, updated_at)| {
                Ok(Session {
                    ...
                    title,
                    model: ModelName::new(&model),
                    created_at: crate::util::parse_rfc3339(&created_at, "session.created_at")?,
                    updated_at: crate::util::parse_rfc3339(&updated_at, "session.updated_at")?,
                    metadata: de_metadata(&metadata)?,
                })
            })
            .collect()
            ...
            .map(
                |(
                    ...
                    Ok(MessageRecord {
                        ...
                        tool_name: None,
                        usage: de_usage(usage.as_deref())?,
                        created_at: crate::util::parse_rfc3339(&created_at, "message.created_at")?,
                        is_compaction: is_compaction != 0,
                        is_summary: is_summary != 0,
                        compaction_meta: de_compaction_meta(compaction_meta.as_deref())?,
                    })
