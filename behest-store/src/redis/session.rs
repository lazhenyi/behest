...
use crate::{MessageRecord, Session, SessionStore, StoreResult};
use behest_core::error::StorageError;
use behest_provider::{ModelName, TokenUsage};
...
impl SessionStore for RedisSessionStore {
    async fn create_session(&self, session: Session) -> StoreResult<Session> {
        ...
        redis::pipe()
            ...
            .hset(
                &key,
                "metadata",
                crate::util::to_json_string(&session.metadata, "session.metadata")?,
            )
    ...
    async fn update_usage(&self, message_id: &Uuid, usage: TokenUsage) -> StoreResult<()> {
        ...
        let session_id = crate::util::parse_uuid(session_id_str, "message_index.session_id")?;
...
async fn load_session_from_redis(
    ...
    let id = crate::util::parse_uuid(id_str, "session.id")?;
    ...
    let created_at = crate::util::parse_rfc3339(created_at_str, "session.created_at")?;
    ...
    let updated_at = crate::util::parse_rfc3339(updated_at_str, "session.updated_at")?;
