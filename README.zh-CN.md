...
| Feature | 说明 |
|---|---|
| `full` | 开箱即用的完整运行时 profile：OpenAI、Anthropic、Redis、Redis Cluster、NATS、PostgreSQL、MongoDB、OpenTelemetry、所有 RAG 后端、所有队列后端和对象存储。刻意不启用 `sqlx-mysql` 或 `sqlx-sqlite`。 |
