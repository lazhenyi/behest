...
impl SessionStore for SqlSessionStore {
    ...
    async fn append_message(&self, message: MessageRecord) -> StoreResult<MessageRecord> {
        ...
        sqlx::query(
            "INSERT INTO messages (id, session_id, role, content, tool_calls, usage, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
        )
        .bind(message.id)
        .bind(message.session_id)
    ...
    async fn list_messages(&self, session_id: &Uuid) -> StoreResult<Vec<MessageRecord>> {
        ...
        >(
            "SELECT id, session_id, role, content, tool_calls, usage, created_at FROM messages WHERE session_id = $1 ORDER BY created_at",
        )
        .bind(*session_id)
        .fetch_all(&self.pool)
        .await
        ...
        rows.into_iter()
            .map(|(id, sid, role, content, tool_calls, usage, created_at)| {
                Ok(MessageRecord {
                    id,
                    session_id: sid,
...
impl SessionStore for SqlSessionStore {
    ...
    async fn append_message(&self, message: MessageRecord) -> StoreResult<MessageRecord> {
        ...
        sqlx::query(
            "INSERT INTO messages (id, session_id, role, content, tool_calls, `usage`, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        )
    ...
    async fn list_messages(&self, session_id: &Uuid) -> StoreResult<Vec<MessageRecord>> {
        ...
        let rows = sqlx::query_as::<_, (String, String, String, String, String, Option<String>, DateTime<Utc>)>(
            "SELECT id, session_id, role, content, tool_calls, `usage`, created_at FROM messages WHERE session_id = ? ORDER BY created_at",
        )
