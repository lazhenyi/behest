...
impl HealthStatus {
    ...
    /// Aggregate a map of named health statuses into a single overall
    /// status using worst-case semantics:
    ///
    /// - If any entry is [`HealthStatus::Unhealthy`], the result is
    ///   `Unhealthy`.
    /// - If any entry is [`HealthStatus::Degraded`] (and none
    ///   unhealthy), the result is `Degraded`.
    /// - Otherwise the result is [`HealthStatus::Healthy`].
    ///
    /// An empty map is considered `Healthy`.
    ///
    /// The reason string lists all non-healthy components so
    /// operators can quickly identify the culprits.
    #[must_use]
    ...
    /// Build a JSON response body suitable for `/healthz` or
    /// `/readyz` HTTP endpoints.
    ///
    /// The shape is:
    /// ```json
    /// {
