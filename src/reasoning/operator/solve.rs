//! Solve operator — solve a specific sub-problem.
...
/// Solves a specific sub-problem.
pub struct SolveOperator {
    base: BaseOperator,
}

impl SolveOperator {
    const SYSTEM_PROMPT: &str =
        "Solve the sub-problem using the available context. Provide a clear solution.";

    /// Creates a new `SolveOperator`.
    #[must_use]
    pub fn new() -> Self {
        Self {
            base: BaseOperator::new(OperatorKind::Solve, "Solve")
                .with_prompt("Solve the specific sub-problem at hand using available information."),
        }
    }
...
impl Default for SolveOperator {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ReasoningOperator for SolveOperator {
