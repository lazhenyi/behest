//! Compose operator — compose partial results into a coherent whole.
...
/// Composes partial results into a coherent whole.
pub struct ComposeOperator {
    base: BaseOperator,
}

impl ComposeOperator {
    const SYSTEM_PROMPT: &str = "Compose the partial results into a coherent whole. Integrate information from multiple sources.";

    /// Creates a new `ComposeOperator`.
    #[must_use]
    pub fn new() -> Self {
        Self {
            base: BaseOperator::new(OperatorKind::Compose, "Compose")
                .with_prompt("Compose partial results into a coherent final output."),
        }
    }
...
#[async_trait]
impl ReasoningOperator for ComposeOperator {
